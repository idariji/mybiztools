...
const VALID_CATEGORIES = [
  'marketing', 'operations', 'salary', 'utilities', 'inventory',
  'rent', 'transport', 'food', 'office_supplies', 'software', 'other',
];
...
export class ExpenseService {
  ...
  static async createExpense(input: CreateExpenseInput): Promise<ServiceResponse> {
    ...
    if (input.budgetId) {
      await prisma.budget.update({
        where: { id: input.budgetId },
        data: { spentAmount: { increment: amountInKobo } },
      });
    }
  ...
  static async getExpenses(
    ...
    return {
      ...
      data: {
        expenses: (expenses as any[]).map(formatExpense),
        pagination: {
          current: page,
          limit,
          total,
  ...
  static async getExpenseById(userId: string, expenseId: string): Promise<ServiceResponse> {
    const expense = await prisma.expense.findFirst({
    ...
    if (!expense) {
      return { success: false, message: 'Expense not found', error: 'EXPENSE_NOT_FOUND' };
    }
  ...
  static async updateExpense(
    ...
    // Adjust budget spentAmount if amount changed and expense is linked
    if (input.amount && expense.budgetId) {
      const newAmount = BigInt(Math.round(input.amount * 100));
      const difference = newAmount - expense.amount;
      await prisma.budget.update({
        where: { id: expense.budgetId },
        data: { spentAmount: { increment: difference } },
      });
    }
    ...
    return {
      success: true,
      message: 'Expense updated successfully',
      data: { expense: formatExpense(updatedExpense) },
    };
  }
  ...
  static async deleteExpense(userId: string, expenseId: string): Promise<ServiceResponse> {
    const expense = await prisma.expense.findFirst({
      where: { id: expenseId, userId },
    }) as any;
    ...
    // Decrement budget spentAmount if linked
    if (expense.budgetId) {
      await prisma.budget.update({
        where: { id: expense.budgetId },
        data: { spentAmount: { decrement: expense.amount } },
      });
    }
    ...
    return {
      ...
      data: {
        summary: {
          ...
          averageExpense: expenses.length > 0 ? totalAmount / expenses.length / 100 : 0,

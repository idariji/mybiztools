      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="max-w-2xl">
            ...
            once: true
          }} className="text-[#FF8A2B] font-bold tracking-wider uppercase text-sm mb-2 block">
              The Problem
            </motion.span>
            <motion.h2 initial={{
            opacity: 0,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          ...
          delay: index * 0.1
              ...
              <Card className={`h-full p-8 group transition-all duration-300 hover:shadow-xl ${problem.border}`}>
                <div className="flex items-start gap-6">
                  <div className={`relative p-4 rounded-2xl bg-gradient-to-br ${problem.gradient} shrink-0 group-hover:scale-110 transition-transform duration-300`}>
                    <problem.icon className={`w-8 h-8 ${problem.color}`} />
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-slate-900 mb-3 group-hover:text-[#FF8A2B] transition-colors">
                      {problem.title}
                    </h3>
                    <p className="text-slate-600 leading-relaxed mb-4">
                      {problem.description}
                    </p>
                    <div className="flex items-center text-sm font-bold text-slate-400 group-hover:text-[#FF8A2B] transition-colors">
